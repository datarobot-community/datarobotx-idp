...
@pytest.fixture
def custom_model_2(cleanup_dr, dr_endpoint, dr_token):
    with cleanup_dr("customModels/"):
        yield get_or_create_custom_model(
            dr_endpoint, dr_token, "pytest custom model", "Regression", target_name="foo"
        )
...
def test_multi_version_idempotency_on_frozen(
    dr_token: str,
    dr_endpoint: str,
    custom_model_2: str,
    cleanup_model_ver: Any,
    sklearn_drop_in_env: str,
    pythonic_runtime_parameters: Any,
    folder_path_with_metadata_and_reqs: Any,
    updated_folder_path_with_metadata_and_reqs: Any,
    cleanup_reg_model_ver: Any,
) -> None:
    _ = get_or_create_custom_model_version(
        endpoint=dr_endpoint,
        token=dr_token,
        custom_model_id=custom_model_2,
        base_environment_id=sklearn_drop_in_env,
        folder_path=folder_path_with_metadata_and_reqs,
    )
    _ = unsafe_get_or_create_custom_model_version_from_previous(
        endpoint=dr_endpoint,
        token=dr_token,
        custom_model_id=custom_model_2,
        base_environment_id=sklearn_drop_in_env,
    ...
    cmv_3 = unsafe_get_or_create_custom_model_version_from_previous(
        endpoint=dr_endpoint,
        token=dr_token,
        custom_model_id=custom_model_2,
        base_environment_id=sklearn_drop_in_env,
        folder_path=folder_path_with_metadata_and_reqs,
        maximum_memory=4048 * 1024 * 1024,
    )
    # get current time
    timestamp = time.time()
    rmv = get_or_create_registered_custom_model_version(
        endpoint=dr_endpoint,
        token=dr_token,
        custom_model_version_id=cmv_3,
        registered_model_name=f"pytest registered model {timestamp}",
    )
    ...
    cmv_3_2 = unsafe_get_or_create_custom_model_version_from_previous(
        dr_endpoint,
        dr_token,
        custom_model_2,
        base_environment_id=sklearn_drop_in_env,
    ...
    rmv_2 = get_or_create_registered_custom_model_version(
        endpoint=dr_endpoint,
        token=dr_token,
        custom_model_version_id=cmv_3_2,
        registered_model_name=f"pytest registered model {timestamp}",
    )
...
def test_multi_version_idempotency(
    dr_token: str,
    dr_endpoint: str,
    custom_model_2: str,
    cleanup_model_ver: Any,
